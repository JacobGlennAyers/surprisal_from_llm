...
def main(sequence, model, tokenizer, id_sequence=None, batch_size=64, no_bos=False, entropy=False, logbase='2',
         **kwargs):
    cols = ['id', 'batch_idx', 'sequ_idx', 'word_idx', 'word', 'surprisal', 'entropy'] if entropy \
        else ['id', 'batch_idx', 'sequ_idx', 'word_idx', 'word', 'surprisal']
    df = pd.DataFrame(columns=cols)
    print("starting")
    processor = sp.batch_process(sequence, model, tokenizer, batch_size=batch_size, no_bos=no_bos, entropy=entropy,
                                 logbase=logbase,
                                 **kwargs)
    ...
    for batch_idx, output in enumerate(processor):
        ...
        for sequ_idx, sequence in enumerate(batch_list):
            for i in range(len(sequence)):
                if entropy:
                    word, surprisal, entropies, _ = sequence[i]
                else:
                    word, surprisal, _ = sequence[i]
                if id_sequence:
...
if __name__ == "__main__":
    ...
    data_in, ids = ut.read_in_csv(arguments.file, arguments.feature_column, arguments.id_column, sep=arguments.sep)
    if arguments.model_class:
        ...
        model = AutoModelForCausalLM.from_pretrained(arguments.model, token=arguments.hf_token).to(arguments.device)
    ...
    data = main(data_in, model, tokenizer, id_sequence=ids, padding=True, keep_space=keep_space,

...
- `--entropy`: Tells module to calculate entropy in addition to surprisal. Whereby entropy is defined as Shannon-Entropy:  $` H(X) :=-\sum\limits_{x\in{\mathcal{X}}} p(x)\log p(x) `$
